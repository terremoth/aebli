...
# (c) Peter Norvig, 2010; See https://norvig.com/lispy2.html
...
_quote, _if, _set, _define, _lambda, _begin, _definemacro, = list(map(Sym,
                                                                      "quote   if   set!  define   lambda   begin   "
                                                                      "define-macro".split()))
...
def repl(prompt='lispy> ', inport=InPort(sys.stdin), out=sys.stdout):
    ...
    while True:
        try:
            ...
            val = evaluate(x)
            if out: 
                print(to_string(val), file=out) if val is not None else print()
...
def add_globals(self):
    ...
    self.update({
        ...
        'port?': lambda x: isa(x, file), 'apply': lambda proc, list_args: proc(*list_args),
        'evaluate': lambda x: evaluate(expand(x)), 'load': lambda fn: load(fn), 'call/cc': callcc,
...
def evaluate(x, env=global_env):
    ...
    while True:
        if isa(x, Symbol):  # variable reference
            ...
            (_, variables, exp) = x
            return Procedure(variables, exp, env)
...
def expand(x, toplevel=False):
    ...
    if not isa(x, list):  # constant => unchanged
        ...
        require(x, (isa(variables, list) and all(isa(v, Symbol) for v in variables))
                or isa(variables, Symbol), "illegal lambda argument list")
...
def let(*args):
    ...
    variables, vals = list(zip(*bindings))
